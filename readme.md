...
The implementation is a small layered Minimal API: the API maps HTTP requests,
the Application layer owns validation and DTO mapping, and Data owns EF Core
repositories and SQLite persistence.
...
SQLite is stored in `VehicleServiceBooking.db` and three service types are
seeded on first startup. Swagger is available at `/swagger` in Development.

## Endpoints

- `GET /service-types` and `GET /service-types/{id}`
- `POST /appointments` and `PUT /appointments/{id}`
- `DELETE /appointments/{id}` (cancellation)

Appointment times are interpreted as local service-centre time. Valid starts
are weekdays at exact hours from 08:00 through 17:00 inclusive. VINs are trimmed
and upper-cased and must contain exactly 17 characters. A slot conflicts only
when both service type and scheduled date-time match; cancellation deletes the
appointment because no status field is part of the brief. Missing records return
404 (the source brief's “401 Not Found” is treated as a typo).
